...
line, = plt.plot(np.zeros(chunk/30 * chunks_per_window)) # get the plot
...
def animate(i): # todo add seconds on x_axis instead of arange
    signal = np.fromstring(stream.read(chunk), 'Int16')[::30]
    global signal_prev
    matrix[i] = running_mean(np.concatenate((signal_prev[-(run_mean_N - 1):], signal)), run_mean_N)
    signal_prev = signal
    line.set_xdata(np.arange(chunk*chunks_per_window/30))
    line.set_ydata(np.hstack(matrix))

    # line.set_ydata(matrix[i])
    # line.set_xdata(np.araxnge(chunk*chunks_per_window))
    if i == 0: #mb remake that shit
        line.set_ydata(np.zeros(chunk/30*chunks_per_window))
    return line,
