...
def animate(i):
    ...
    # arr_n[i] = sum(signal) / 
    arr_n[i] = sum(signal) / float(chunk)
    # for k in xrange(j):
        # arr[i] = shrink(signal, j)[k]

    line.set_ydata(arr_n)
    line.set_xdata(np.arange(len(arr_n)))
    if i == 0:
...
ani = animation.FuncAnimation(fig, animate, np.arange(window), interval=5, blit=True)
plt.show()
stream.stop_stream()
stream.close()
p.terminate()